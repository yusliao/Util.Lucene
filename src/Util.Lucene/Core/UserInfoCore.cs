...
{
    public class UserInfoCore: IndexOptionBase<UserInfo>
    {
        public const string indexPath = "UserInfo";
        private static Queue<UserInfo> cardRecordQueue = new Queue<UserInfo>();
        ...
        public override void Insert(UserInfo t)
        {
            cardRecordQueue.Enqueue(t);
        }

        public override void Modify(UserInfo t)
        {
            cardRecordQueue.Enqueue(t);
        }

        public override void Del(UserInfo t)
        {
            cardRecordQueue.Enqueue(t);
        }


        public override List<UserInfo> Search(string keyword, out int recordCount, int pageindex = 1, int pagesize = 100)
        {
            ...
            using (ParallelMultiSearcher searcher = new ParallelMultiSearcher(new IndexSearcher(directory, true)))
            {
                ...
                watch.Stop();
                RaiseIndexOptionMsgCallBack(new IndexOptionEventArgs<UserInfo>(null, true, watch.ElapsedMilliseconds, null, tds.TotalHits.ToString(), 2));

            }
            return modelList;
        }
        public override List<UserInfo> Search(UserInfo record, out int recordCount, int pageindex = 1, int pagesize = 100)
        {
            List<UserInfo> modelList = new List<UserInfo>();
            using (ParallelMultiSearcher searcher = new ParallelMultiSearcher(new IndexSearcher(directory, true)))
            {
                ...
                {
                    RaiseIndexOptionMsgCallBack(new IndexOptionEventArgs<UserInfo>(null, false, 0, messageType: 2, error: "请输入要查询的内容"));
                    recordCount = 0;
                    return modelList;
                }
        ...
        public List<UserInfo> BusinessSearch(UserInfo record)
        {
            List<UserInfo> modelList = new List<UserInfo>();
            using (ParallelMultiSearcher searcher = new ParallelMultiSearcher(new IndexSearcher(directory, true)))
            {
                ...
                if (tds.TotalHits > 0)
                {
                    ...
                    {
                        watch.Stop();
                        RaiseIndexOptionMsgCallBack(new IndexOptionEventArgs<UserInfo>(null, false, watch.ElapsedMilliseconds, ex.Message, tds.TotalHits.ToString(), 2));
                        return modelList;
                    }
        ...
        //public List<UserInfo> BusinessExportSearch(UserInfo record)
        //{
        //    List<UserInfo> modelList = new List<UserInfo>();
        //    using (ParallelMultiSearcher searcher = new ParallelMultiSearcher(new IndexSearcher(directory, true)))
        //    {
        ...
        protected override void CRUDIndex()
        {
            ...
            while (cardRecordQueue.Count > 0)
            {
                ...
                UserInfo Model = cardRecordQueue.Dequeue();
                try
                {
                ...
                {
                    RaiseIndexOptionMsgCallBack(new IndexOptionEventArgs<UserInfo>(Model, false, null, ex.Message, this.GetDocCount().ToString()));
                }
            }
            writer.Dispose();
            directory.Dispose();
